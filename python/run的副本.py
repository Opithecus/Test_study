...
res_map = {'10056': '茅台1935', '2476': '贵州茅台酒（壬寅虎年）', '2478': '贵州茅台酒（珍品）',
           '2475': '贵州茅台酒（壬寅虎年）'}
...
if __name__ == '__main__':
    ...
    get_map()
    # 省份,城市,设备id,token,MT-Token-Wap(抓包有)
    # 当前城市随机申购，不支持指定,有能力可以改下，反馈出来
    #青龙面板加入环境变量mtec_user和MTTokenD
    #mtec_user是要通知的用户id，MTTokenD是茅台预约参数，多个请换行，格式'省,市,token,MT-Token-Wap'，无省份参考：'北京市,北京市,token,MT-Token-Wap'
    ...
    for mt_token in mt_token_list:
        if not mt_token:
            continue
        province, city, device_id, token, ck = mt_token.split(',')
        print("device_id=", device_id, "开始任务")
        try:
            ...
            for itemCode in itemCodes:
                ...
                shop_id = get_shop_item(sessionId, itemCode, device_id, token, province, city)
                res = mt_add(itemCode, str(shop_id), sessionId, user_id, token, device_id)
                s += userName + '_' + mobile + '_' + itemCode + '_' + name + '---------------' + res + '\n'
            r = getUserEnergyAward(device_id, ck)
            s += userName + '_' + mobile + '---------------' + r + '\n              \n'
        except Exception as e:
            print(e)
        print("device_id=", device_id, "结束任务")
        print(s)
    tongzhi(s)
